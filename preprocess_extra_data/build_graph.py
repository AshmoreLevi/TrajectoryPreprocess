...
def build_global_POI_checkin_graph(dst_dir, new2raw_rid, raw2new_rid, raw_rn_dict, mbr, grid_size):
    G = nx.DiGraph()
        ...
        for traj in trajs:
            ...
            for i in range(len(tmp_pt_list)):
                ...
                if candi_pt is not None:   #找到第一个不为空的节点
                    pre_id = str(raw2new_rid[str(candi_pt.eid)]) #记录u的eid # 这里发现MM-STGED匹配好的是newid 而我们直接得到的是rawid 进行转化
                    first_flag = i    
            ...
            for i in range(first_flag+1, len(tmp_pt_list)):
                ...
                if candi_pt is not None:
                    u = str(pre_id)
                    v = str(raw2new_rid[str(candi_pt.eid)]) # 这里发现MM-STGED匹配好的是newid 而我们直接得到的是rawid 进行转化
...
if __name__ == '__main__':
    city = 'Chengdu'
    dst_dir = f'../data/final_data/{city}/train_data/'
    json_dir = f'../data/map/{city}/extra_data/'
    with open(json_dir + 'new2raw_rid.json', "r") as f:
        new2raw_rid = json.load(f)
    with open(json_dir + 'raw2new_rid.json', "r") as f:
        raw2new_rid = json.load(f)
    ...
    G = build_global_POI_checkin_graph(dst_dir, new2raw_rid, raw2new_rid, raw_rn_dict, mbr, grid_size)
    nodelist = G.nodes()
    print(len(nodelist))
    ...
    # Save graph to disk
    save_G_path = f'../data/map/{city}/extra_data/'
